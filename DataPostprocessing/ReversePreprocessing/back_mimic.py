...
path = "../../Annotations/Preprocessed/MIMIC-CXR-JPG.csv"
path2 = ... # path to paddings.csv

df = pd.read_csv(path)
pads = pd.read_csv(path2)

pads.filename = pads.filename.str.split('/').str[-1].str.split('.').str[0]
pads.set_index('filename', inplace=True)

new_df = pd.DataFrame(columns=df.columns)
new_df.to_csv("../../Annotations/OriginalResolution/MIMIC-CXR-JPG.csv", index=False)
...
for index in range(0, len(df)):
    ...
    if index % 20000 == 0 and index != 0:
        new_df.to_csv("../../Annotations/OriginalResolution/MIMIC-CXR-JPG.csv", mode='a', header=False, index=False)
